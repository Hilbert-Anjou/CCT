...
class StageTranNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, conv_size, output_dim, levels, dropconnect=0., dropout=0., dropres=0.):
        ...
        self.nn_scale = nn.Linear(7200, 3600)
        self.nn_rescale = nn.Linear(3600, 1800)
        self.nn_output = nn.Linear(1800, 400)
        
        #self.nn_avg = Linear_BBB(24, 1)
        self.nn_avg = nn.Linear(400, 1)
    ...
    def forward(self, input, device=torch.device("cuda:0" if torch.cuda.is_available() == True else 'cpu')):
        ...
        #transmat = torch.zeros(batch_size,24, 18432).to(device)
        for t in range(24):   # to process the input data based on each timestep (variable t) 
            ...
            #transmat= torch.cat((transmat[:,1:,:], encode.view(batch_size,1,18432)), 1)

        #transmat = self.CCT(transmat.view(batch_size,3,384,384)).view(batch_size,24,200)
        transmat= self.CCT(transmat.view(batch_size,3,224,224)).view(batch_size,-1)


        
        local_theme = self.nn_scale(transmat)                          # input: hidden_dim, output: hidden_dim //6
        local_theme = torch.relu(local_theme) 
        local_theme = self.nn_rescale(local_theme)                        # input: hidden_dim//6, output: hidden_dim
        local_theme = torch.relu(local_theme)                          # the size of local_h, this is the attention part
        # before conv, the size is batch_size by hidden_dim by conv_size

          

        rnn_output = local_theme

        
        #rnn_output = torch.cat([rnn_output,origin_single_h],dim=1)

            # the size now is batch_size*timestep by hidden_dim
        ...
        output = self.nn_avg(output)
        output = torch.sigmoid(output).view(batch_size)
        return output
...
class PatchUpModel(nn.Module):
    ...
    def hook_modify(self, module, input, output):
        output=output.view(output.shape[0],1,24,300)
...
class SaveFeatures:
    def __init__(self, module):
        self.hook_f = module.register_forward_hook(self.hook_fn_forward)
       # self.hook_b = module.register_full_backward_hook(self.hook_fn_backward)
        self.features = None
        #self.gradients = None
