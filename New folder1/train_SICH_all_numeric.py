...
def parse_arguments(parser):
    ...
    parser.add_argument('--batch_size', type=int, default=300, help='Training batch size') # change the batch size smaller in order not to exceed memory
    #parser.add_argument('--batch_size', type=int, default=128, help='Training batch size')
    parser.add_argument('--epochs', type=int, default=50, help='Training epochs')
    parser.add_argument('--lr', type=float, default=5e-7, help='Learing rate')#5e-5
        ...
        for train, val in kf.split(train_data):
            ...
            for epoch in range(50):
                ...
                #for batch_x, batch_y, batch_discharge in loader:                               # for each batch
                    ...
                    batch_mortality_output, H ,S= model(batch_x, batch_time, device)

                    

                    #loss_numu = batch_y * torch.log(batch_mortality_output + 1e-7) + (1 - batch_y) * torch.log(1 - batch_mortality_output + 1e-7)
                    loss_numu =0.2*batch_y * torch.square(1-batch_mortality_output)*torch.log(batch_mortality_output) + 0.8*(1 - batch_y) *torch.square(batch_mortality_output)* torch.log(1 - batch_mortality_output )
                ...
                with torch.no_grad():
                    ...
                    val_mortality_output, _ ,_= model(x_val, val_time, device)
                    ...
                    test_mortality_output, _ ,_= model(x_test, test_time, device)
