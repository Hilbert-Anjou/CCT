...
class StageTranNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, conv_size, output_dim, levels, dropconnect=0., dropout=0., dropres=0.3):
        ...
        self.extraction=nn.Linear(825,512)
        ...
        self.vanilla=FTTransformer(categories = (24,),      # tuple containing the number of unique values within each category
                                    num_continuous = 512,                # number of continuous values
                                    dim = 128,                           # dimension, paper set at 32
    ...
    def forward(self, input, time, device):
        ...
        transmat = torch.zeros(batch_size,24, 512).to(device)
        ...
        for t in range(time_step):   
            ...
            transmat= torch.cat((transmat[:,1:,:], trans.view(batch_size,1,512)), 1)
            transmat=self.vanilla(transmat,catgory)
