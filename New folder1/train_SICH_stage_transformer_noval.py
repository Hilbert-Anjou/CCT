...
from model_SICH_stage_transformer_sep import StageTranNet
...
if __name__ == '__main__':
    ...
    if args.test_mode == 1:
        ...
        for train, val in kf.split(train_data):
            ...
            #model = torch.load('model/stage_tran_100ep_seed12345.pt')
            ...
            optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr,amsgrad=False,weight_decay=0.005)
            ...
            for epoch in range(50):
                ...
                for batch_x, batch_y in loader:
                    ...
                    batch_mortality_output_0 = torch.where((batch_mortality_output*(1-batch_y)) < 0.35, zero, batch_mortality_output*(1-batch_y))
                ...
                with torch.no_grad():
                    ...
                    train_mortality_output= model(x_train, None, device)
                    train_mortality_pred = train_mortality_output.data.cpu().numpy().squeeze()
                    train_mortality_true=y_train
                    print('trainning set')
                    train_mortality_ret, optimal_proba_cutoff = metrics.print_metrics_binary(train_mortality_true, train_mortality_pred, verbose = False,cut_off = None)
                    print(train_mortality_ret)

                    x_val = torch.as_tensor(x_val, dtype = torch.float32).to(device)
                    val_mortality_output= model(x_val, None, device)
                    ...
                    test_mortality_ret, _ = metrics.print_metrics_binary(val_mortality_true, val_mortality_pred, verbose = False,cut_off = 0.5)
        ...
        torch.save(best_model[np.argmax(auroc_mortality)], 'model/temp2.pt')
        ...
        for train, val in kf.split(train_data):
            ...
            #optimizer = torch.optim.Adam(model.parameters(), lr=args.lr,amsgrad=True)
            optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr,amsgrad=True)
            #scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5, min_lr=1e-8)
            #scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[10,28,55], gamma=0.1, last_epoch=-1)
            scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[10,28,60], gamma=0.1, last_epoch=-1)
            x_train = train_data[train]
            y_train = train_flag[train]
