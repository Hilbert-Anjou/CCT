...
from model_SICH_stage_seq import StageTranNet,PatchUpModel
from model_SICH_stage_transformer import MixTranNet
import os
...
if __name__ == '__main__':
    ...
    if args.test_mode == 1:
        ...
        for train, val in kf.split(train_data):
            ...
            #model = torch.load('model/patchup_0.929.pt')
            #model = torch.load('model/mixupsamples.pt')
            #model = torch.load('model/patchupsoft_0.935.pt')
            ...
            for epoch in range(66):
                ...
                for batch_x, batch_y in loader:
                    ...
                    """
                    batch_mortality_output,mixout,mix_label= model(batch_x, batch_y, device)
                    loss_mix = torch.log(1-torch.abs(mixout-mix_label) + 1e-7)
                    """
                    ...
                    loss_mortality = torch.neg(torch.sum(loss_mortality))+loss
