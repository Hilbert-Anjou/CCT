...
def parse_arguments(parser):
    ...
    parser.add_argument('--lr', type=float, default=5e-6, help='Learing rate')#5e-5
...
if __name__ == '__main__':
    ...
    #
        ...
        model_se = torch.load('model/model1.pt')
        model_sp = torch.load('model/model4.pt')
        model_se.eval()
        model_sp.eval()
        for train, val in kf.split(train_data):
            ...
            for epoch in range(5):
                ...
                #for batch_x, batch_y, batch_discharge in loader:                               # for each batch
                    ...
                    #batch_mortality_output, H = model(batch_x, batch_time, device)

                    batch_mortality_output_se, H ,S= model_se(batch_x, batch_time, device)
                    batch_mortality_output_sp, H ,S= model_sp(batch_x, batch_time, device)
                    batch_mortality_output_se = batch_mortality_output_se.to(device)
                    batch_mortality_output_sp = batch_mortality_output_sp.to(device)
                    
                    batch_mortality_output=model(batch_mortality_output_se,batch_mortality_output_sp)

                    #loss_numu = batch_y * torch.log(batch_mortality_output + 1e-7) + (1 - batch_y) * torch.log(1 - batch_mortality_output + 1e-7)
                    loss_numu =0.7*batch_y * torch.square(1-batch_mortality_output)*torch.log(batch_mortality_output) + 0.3*(1 - batch_y) *torch.square(batch_mortality_output)* torch.log(1 - batch_mortality_output )
                ...
                with torch.no_grad():
                    ...
                    #val_mortality_output, _ = model(x_val, val_time, device)
                    val_mortality_output_se, _ ,_= model_se(x_val, val_time, device)
                    val_mortality_output_sp, _ = model_sp(x_val, val_time, device)
                    val_mortality_output_se = val_mortality_output_se.to(device)
                    val_mortality_output_sp = val_mortality_output_sp.to(device)
                    val_mortality_output=model(val_mortality_output_se,val_mortality_output_sp)
                    ...
                    #test_mortality_output, _ = model(x_test, test_time, device)
                    test_mortality_output_se, _ ,_ = model_se(x_test, test_time, device)
                    test_mortality_output_sp, _ ,_ = model_sp(x_test, test_time, device)
                    test_mortality_output_se = test_mortality_output_se.to(device)
                    test_mortality_output_sp = test_mortality_output_sp.to(device)
                    test_mortality_output=model(test_mortality_output_se,test_mortality_output_sp)
                    ...
                    if (test_mortality_ret["auroc"] > auroc_mortality[count]):
                        ...
                        torch.save(model, 'model/temp.pt')
                    """
                    if epoch % CEsingle==0:
