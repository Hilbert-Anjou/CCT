...
from simpnet import SimpNet, PredictNet
...
def parse_arguments(parser):
    ...
    parser.add_argument('--batch_size', type=int, default=146, help='Training batch size') # change the batch size smaller in order not to exceed memory
...
if __name__ == '__main__':
    ...
    if args.test_mode == 1:
        ...
        train_data_loader = common_utils_SICH_all_numeric.DeepSupervisionDataLoader(dataset_dir=os.path.join(
    ...
    #
        ...
        test_data_loader = common_utils_SICH_all_numeric.DeepSupervisionDataLoader(dataset_dir=os.path.join(args.data_path, 'test'),
                                                                        listfile=os.path.join(args.data_path, 'test_listfile.csv'), small_part=args.small_part)
        test_data_gen = utils_SICH_all_numeric.BatchGenDeepSupervision(test_data_loader, discretizer,
        ...
        #SICH_data = data_gen[0][:, :, 0 : args.data_dim]                          # all data part
        
        #SICH_data = np.insert(SICH_data, -1, -1, axis = 2)                    # insert '-1' since we delete 1 variable 'discharge needs'
        
        #SICH_data = data_gen[0]
        #flag = np.array(data_gen[1])                                          # binary flags for mortality
        #discharge_flag = np.array(data_gen[2])                                # flag for discharge location
        
        #mask_all = data_gen[0][:, :, args.data_dim:]                            # mask for recorded data
        fold=10
        ...
        MSEloss=np.ones(fold)*np.inf
        #sensitivity_discharge = np.zeros(10)
        #specificity_discharge = np.zeros(10)
        #accuracy_discharge = np.zeros(10)
        #auroc_discharge = np.zeros(10)
        ...
        for train, val in kf.split(train_data):
            ...
            model_p = StageNet(args.input_dim, args.rnn_dim, args.K, args.output_dim, args.chunk_level, args.dropconnect_rate, args.dropout_rate, args.dropres_rate).to(device)
            model_p.load_state_dict(torch.load('model/model1_dict.pt'))
            model = PredictNet(batch_size=args.batch_size).to(device)
            #model.load_state_dict(torch.load('model/predict1.pt'))
            #optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
            optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
            #scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40,70,90], gamma=0.5, last_epoch=-1)
            #loss_function = nn.MSELoss()
            loss_function = SoftDTW(use_cuda=True, gamma=0.1)   #https://github.com/Maghoumi/pytorch-softdtw-cuda
            ...
            for epoch in range(20):
                ...
                #for batch_x, batch_y, batch_discharge in loader:                               # for each batch
                    ...
                    batch_mortality_output, H ,S= model_p(batch_x, batch_time, device)
                    
                    with torch.autograd.set_detect_anomaly(True):
                        
                        tempH=torch.zeros(batch_x.size(0),1,840).to(device)
                        tempS=torch.zeros(batch_x.size(0),1,1).to(device)
                        tempH_label=torch.zeros(batch_x.size(0),1,840).to(device)
                        tempS_label=torch.zeros(batch_x.size(0),1,1).to(device)
                        train_sequence=model.create_inout_sequences(H,S)
                        model.hidden_cellS = (torch.zeros(1,batch_x.size(0),model.hidden_layer_size).cuda(),
                        torch.zeros(1,batch_x.size(0),model.hidden_layer_size).cuda()) # (num_layers * num_directions, batch_size, hidden_size)
                        model.hidden_cellH = (torch.zeros(1,batch_x.size(0),model.hidden_layer_size).cuda(),
                        torch.zeros(1,batch_x.size(0),model.hidden_layer_size).cuda()) # (num_layers * num_directions, batch_size, hidden_size)
                        for h_in,h_label,s_in,s_label in  train_sequence:
                            
                            
                            predictionH,predictionS=model(h_in,s_in)
                            tempH=torch.cat([tempH,predictionH],dim=1)
                            tempS=torch.cat([tempS,predictionS],dim=1)
                            tempH_label=torch.cat([tempH_label,predictionH],dim=1)
                            tempS_label=torch.cat([tempS_label,predictionS],dim=1)
                            #soft_DTW =soft_DTW+loss_function(predictionH, h_label)+loss_function(predictionS, s_label)
                            #print(soft_DTW.shape)
                        print(tempH[:,1:,:].shape)
                        #normalization
                        """
                        Hmin=torch.min(torch.min(tempH[:,1:,:],tempH_label[:,1:,:]),dim=1,keepdim=True)[0]
                        Hmax=torch.max(torch.max(tempH[:,1:,:],tempH_label[:,1:,:]),dim=1,keepdim=True)[0]
                        Smin=torch.min(torch.min(tempS[:,1:,:],tempS_label[:,1:,:]),dim=1,keepdim=True)[0]
                        Smax=torch.max(torch.max(tempS[:,1:,:],tempS_label[:,1:,:]),dim=1,keepdim=True)[0]
                        H_loss=loss_function((tempH[:,1:,:]-Hmin)/(Hmax-Hmin), (tempH_label[:,1:,:]-Hmin)/(Hmax-Hmin))
                        S_loss=loss_function((tempS[:,1:,:]-Smin)/(Smax-Smin), (tempS_label[:,1:,:]-Smin)/(Smax-Smin))
                        """
                        
                        H_loss=loss_function(tempH[:,1:,:], tempH_label[:,1:,:])-0.5*(loss_function(tempH[:,1:,:], tempH[:,1:,:])+loss_function(tempH_label[:,1:,:], tempH_label[:,1:,:]))
                        S_loss=loss_function(tempS[:,1:,:], tempS_label[:,1:,:])-0.5*(loss_function(tempS[:,1:,:], tempS[:,1:,:])+loss_function(tempS_label[:,1:,:], tempS_label[:,1:,:]))
                        print(H_loss,S_loss)
                        soft_DTW=H_loss.mean()+S_loss.mean()
                        print('Soft DtW for',soft_DTW) 
                        optimizer.zero_grad()
                        soft_DTW.backward(retain_graph=True)
                        #soft_DTW.backward(retain_graph=True)
                        optimizer.step()
                    ...
                    #loss_numu =0.8*batch_y * torch.square(1-batch_mortality_output)*torch.log(batch_mortality_output) + 0.2*(1 - batch_y) *torch.square(batch_mortality_output)* torch.log(1 - batch_mortality_output )
                ...
                with torch.no_grad():
                    ...
                    val_mortality_output, H_val ,S_val = model_p(x_val, val_time, device)
                    h_val,h_vallabel=model.create_inout_sequences(H_val)
                    s_val,s_vallabel=model.create_inout_sequences(S_val)
                    predict_valH,predict_valS=model(h_val,s_val)
                    Val_MSE = loss_function(predict_valH, h_vallabel)+loss_function(predict_valS, s_vallabel)
                    ...
                    #val_mortality_ret, optimal_proba_cutoff = metrics.print_metrics_binary(val_mortality_true, val_mortality_pred, verbose = True,cut_off = None)
                    ...
                    test_mortality_output, H_test ,S_test= model_p(x_test, test_time, device)
                    test_sequence=model.create_inout_sequences(H_test,S_test)
                    temp_testH=torch.zeros(x_test.size(0),1,840).to(device)
                    temp_testS=torch.zeros(x_test.size(0),1,1).to(device)
                    testH_label=torch.zeros(x_test.size(0),1,840).to(device)
                    testS_label=torch.zeros(x_test.size(0),1,1).to(device)
                    model.hidden_cellS = (torch.zeros(1,x_test.size(0),model.hidden_layer_size).cuda(),
                    torch.zeros(1,x_test.size(0),model.hidden_layer_size).cuda()) # (num_layers * num_directions, batch_size, hidden_size)
                    model.hidden_cellH = (torch.zeros(1,x_test.size(0),model.hidden_layer_size).cuda(),
                    torch.zeros(1,x_test.size(0),model.hidden_layer_size).cuda()) # (num_layers * num_directions, batch_size, hidden_size)
                    for h_test,h_testlabel,s_test,s_testlabel in test_sequence:
                            
                        predict_testH,predict_testS=model(h_test,s_test)
                        temp_testH=torch.cat([temp_testH,predict_testH],dim=1)
    
                        temp_testS=torch.cat([temp_testS,predict_testS],dim=1)
                        
                        testH_label=torch.cat([testH_label,h_testlabel],dim=1)
                        testS_label=torch.cat([testS_label,s_testlabel],dim=1)
                    
                    test_DTW = (loss_function(temp_testH[:,1:,:], testH_label[:,1:,:])+loss_function(temp_testS[:,1:,:], testS_label[:,1:,:])).mean()
                    ...
                    #test_mortality_ret, _ = metrics.print_metrics_binary(test_mortality_true, test_mortality_pred, verbose = True,cut_off = optimal_proba_cutoff)
                    """
                    ...
                    """
                    if (test_DTW < MSEloss[count]):
                        if test_DTW<np.min(MSEloss):
                            torch.save(model.state_dict(), 'model/predict_temp.pt')
                        MSEloss[count]=test_DTW
                        with open('LSTMprediction//for fold'+str(count)+".txt","a") as f:
                            f.write('for epoch'+str(epoch)+':\n')
                            f.write('DTWLoss'+str(test_DTW))
