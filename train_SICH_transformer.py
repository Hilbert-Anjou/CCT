...
from model_SICH_transformer import TransNet
...
def parse_arguments(parser):
    ...
    parser.add_argument('--batch_size', type=int, default=4, help='Training batch size') # change the batch size smaller in order not to exceed memory
...
if __name__ == '__main__':
    ...
    if args.test_mode == 1:
        ...
        for train, val in kf.split(train_data):
            ...
            model = TransNet(args.input_dim, args.rnn_dim, args.K, args.output_dim, args.chunk_level, args.dropconnect_rate, args.dropout_rate, args.dropres_rate).to(device)
            #model = torch.load('model/transformers.pt')
