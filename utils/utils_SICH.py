# -*- coding: utf-8 -*-
"""
Created on Mon Jul 18 22:14:49 2022
...
def load_data(reader, discretizer, small_part=False, return_names=False):
    ...
    if small_part:
        N = 1000
    ret = common_utils_SICH.read_chunk(reader, N)
    data = ret["X"]
    ts = ret["t"]
    labels = ret["y"]
    names = ret["name"]
...
def save_results(names, ts, pred, y_true, path):
    common_utils_SICH.create_directory(os.path.dirname(path))
